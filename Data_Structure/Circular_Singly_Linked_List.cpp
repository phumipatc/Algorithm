/*
	Task		: Circular Singly Linked List
	Author		: Phumipat C.
	Language	: C++
	Explanation	: 
*/
...
void insert_node_after(Node *prev_node, int val, Node *head){
	if(prev_node == NULL){
...
Node *move_to_end(Node *node_to_move, Node *head){
	if(node_to_move == NULL){
		cout << "The given node cannot be NULL";
		return node_to_move;
	}

	while(node_to_move->next != head){
...
int main()
{
	...
	// Linked list: 1 2
	insert_node_after(current, 2, head);

	// current pointing to 2
	current = move_forward(current);

	// Linked list: 1 2 3
	insert_node_after(current, 3, head);

	// current pointing to 1
	current = move_to_front(current, head);

	// Linked list: 1 4 2 3
	insert_node_after(current, 4, head);

	// current pointing to 3
	current = move_to_end(current, head);

	// Linked list: 1 4 2 3 5
	insert_node_after(current, 5, head);
